...
@RestController @RequestMapping("/api/planner") public class PlannerController {
 ...
 @GetMapping("/today") List<TaskResponse> today(@AuthenticationPrincipal String id){return map(tasks.findByUser_IdAndDueDateOrderByDoneAsc(id,LocalDate.now()));}
 @GetMapping("/upcoming") List<TaskResponse> upcoming(@AuthenticationPrincipal String id){return map(tasks.findByUser_IdAndDueDateGreaterThanOrderByDueDateAsc(id,LocalDate.now()));}
 ...
 @PutMapping("/tasks/{taskId}") TaskResponse update(@AuthenticationPrincipal String id,@PathVariable String taskId,@RequestBody TaskRequest r){StudyTask t=tasks.findByIdAndUser_Id(taskId,id).orElseThrow(()->new IllegalArgumentException("Task not found."));apply(t,r);return TaskResponse.from(tasks.save(t));}
 @DeleteMapping("/tasks/{taskId}") void delete(@AuthenticationPrincipal String id,@PathVariable String taskId){tasks.delete(tasks.findByIdAndUser_Id(taskId,id).orElseThrow(()->new IllegalArgumentException("Task not found.")));}
